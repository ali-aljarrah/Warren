<!-- <footer>
    <div class="container">
        <div class="row">
            <div class="col-md-6 order-2 order-md-1">
                <div class="row">
                    <div class="col-md-6 mb-5">
                        <div>
                            <p class="white-color fs-20 fw-600">Our Services</p>
                            <div>
                                <a class="fs-16 white-color text-decoration-none d-block mb-2" href="/dental-and-medical-it">Dental & Medical IT</a>
                                <a class="fs-16 white-color text-decoration-none d-block mb-2" href="/web-development">Web Development</a>
                                <a class="fs-16 white-color text-decoration-none d-block mb-2" href="/machine-learning">Machine Learning</a>
                                <a class="fs-16 white-color text-decoration-none d-block mb-2" href="/digital-marketing">Digital Marketing</a>
                                <a class="fs-16 white-color text-decoration-none d-block mb-2" href="/smart-home">Smart Home</a>
                                <a class="fs-16 white-color text-decoration-none d-block mb-2" href="/emr-development">EMR Development</a>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-6 mb-5">
                        <div>
                            <p class="white-color fs-20 fw-600">Social</p>
                            <div>
                                <a class="fs-16 white-color text-decoration-none d-block mb-2" href="#">Facebook</a>
                                <a class="fs-16 white-color text-decoration-none d-block mb-2" href="#">YouTube</a>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-6 mb-5">
                        <div>
                            <p class="white-color fs-20 fw-600">Contact</p>
                            <div>
                                <a class="fs-16 white-color text-decoration-none d-block mb-2" href="tel:+000000">+1 248 233 3355</a>
                                <a class="fs-16 white-color text-decoration-none d-block mb-2" href="mailto:info@URL.COM">info@itrus.us</a>
                                <p class="fs-16 white-color">
                                Address
                                </p>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            <div class="col-md-6 order-1 order-md-2 mb-5 mb-md-0">
                <div class="text-start text-lg-end ps-md-5 ps-0 ms-md-5 ms-0">
                    
                    <div>
                        <a class="fs-16 white-color fw-600 text-decoration-none hover-link" href="/contact-us">
                            <span>Get started</span>
                            <span>
                                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                                    <path d="M15.71 12.71C15.801 12.6149 15.8724 12.5028 15.92 12.38C16.02 12.1365 16.02 11.8635 15.92 11.62C15.8724 11.4973 15.801 11.3851 15.71 11.29L12.71 8.29C12.5217 8.1017 12.2663 7.99591 12 7.99591C11.7337 7.99591 11.4783 8.1017 11.29 8.29C11.1017 8.47831 10.9959 8.7337 10.9959 9C10.9959 9.26631 11.1017 9.5217 11.29 9.71L12.59 11H9C8.73479 11 8.48043 11.1054 8.2929 11.2929C8.10536 11.4804 8 11.7348 8 12C8 12.2652 8.10536 12.5196 8.2929 12.7071C8.48043 12.8946 8.73479 13 9 13H12.59L11.29 14.29C11.1963 14.383 11.1219 14.4936 11.0711 14.6154C11.0203 14.7373 10.9942 14.868 10.9942 15C10.9942 15.132 11.0203 15.2627 11.0711 15.3846C11.1219 15.5064 11.1963 15.617 11.29 15.71C11.383 15.8037 11.4936 15.8781 11.6154 15.9289C11.7373 15.9797 11.868 16.0058 12 16.0058C12.132 16.0058 12.2627 15.9797 12.3846 15.9289C12.5064 15.8781 12.617 15.8037 12.71 15.71L15.71 12.71ZM22 12C22 10.0222 21.4135 8.08879 20.3147 6.4443C19.2159 4.79981 17.6541 3.51809 15.8268 2.76121C13.9996 2.00433 11.9889 1.8063 10.0491 2.19215C8.10929 2.578 6.32746 3.53041 4.92894 4.92894C3.53041 6.32746 2.578 8.10929 2.19215 10.0491C1.8063 11.9889 2.00433 13.9996 2.76121 15.8268C3.51809 17.6541 4.79981 19.2159 6.4443 20.3147C8.08879 21.4135 10.0222 22 12 22C14.6522 22 17.1957 20.9464 19.0711 19.0711C20.9464 17.1957 22 14.6522 22 12ZM4 12C4 10.4178 4.4692 8.87104 5.34825 7.55544C6.2273 6.23985 7.47673 5.21447 8.93854 4.60897C10.4003 4.00347 12.0089 3.84504 13.5607 4.15372C15.1126 4.4624 16.538 5.22433 17.6569 6.34315C18.7757 7.46197 19.5376 8.88743 19.8463 10.4393C20.155 11.9911 19.9965 13.5997 19.391 15.0615C18.7855 16.5233 17.7602 17.7727 16.4446 18.6518C15.129 19.5308 13.5823 20 12 20C9.87827 20 7.84344 19.1572 6.34315 17.6569C4.84286 16.1566 4 14.1217 4 12Z" fill="#F6FAFD"/>
                                </svg>
                            </span>
                        </a>
                    </div>
                </div>
            </div>
        </div>
        <div class="row mt-0 mt-md-5 mb-3 pt-5">
            <div class="col-md-6 order-2 order-md-1">
                <div class="text-center text-md-start">
                    <div class="fs-16 white-color">
                    Copyright URL <span id="current-date"></span>. All rights reserved.
                    </div>
                </div>
            </div>
            <div class="col-md-6 order-1 order-md-1">
                <div class="text-center text-md-end mb-4 mb-md-0">
                    <a class="fs-16 white-color text-decoration-none me-5" href="/terms-and-conditions">Terms & Conditions</a>
                    <a class="fs-16 white-color text-decoration-none" href="/privacy-policy">Privacy Policy</a>
                </div>
            </div>
        </div>
    </div>
</footer> -->

<script src="/assets/js/bootstrap.bundle.min.js" defer></script>
<script src="/assets/js/script.js" defer></script>